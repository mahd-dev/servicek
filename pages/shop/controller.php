<?php
	...
	if(isset($_POST["email"]) && isset($_POST["subject"]) && isset($_POST["message"])){
		...
		$mail->addAddress($shop->url."@servicek.net");
	...
	if ($user!=null && ($shop->admin==$user || $user->is_master)) {
		if (isset($_POST['element']) && isset($_POST['pk']) && isset($_POST['name']) && isset($_POST['value'])) {
			...
			$shop->transform_to($_POST["transform"]);
			die("success");
			...
			$new_category->parent = $shop->categories[0];
			die(json_encode(array("status"=>"success",
        ...
        $ogp->setURL( url_root."/".urlencode($shop->url) );
        ...
        if($seo_img){
            ...
            $image->setSecureURL( str_replace("http://", "https://", url_root.$paths->shop_image->url.$seo_img) );
            $image->setType( 'image/jpeg' );
            $ogp->addImage($image);
