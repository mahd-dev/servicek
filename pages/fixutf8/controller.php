<?php
  ...
  function escapestr($value, $url = false){
    global $db;
    if($url){
      $value = str_replace(array(" ", ":", "/", "\\", "&", "\"", "'", "(", ")", "-", "_", "", "", "=", "*", "°", "+", "~", "¬", "¹", "#", "{", "}", "[", "]", "|", "`", "^", "@", "<", ">", ",", ".", ":", "!", "%", "$", "?", "§"), "", str_replace(array("é", "è", "ê", "ë", "Ã"), "e", str_replace(array("ï", "î"), "i", str_replace(array("à", "â", "ä"), "a", str_replace(array("ç"), "c", str_replace(array("ù"), "u", str_replace(array("ô", "ö"), "o", strtolower($value))))))));
    }
