        ...
        public function __get($name){
            ...
            if ($this->id != NULL) {
                switch($name){
                    ...
                    case "categories":
                        $list = array();
                        $q=$db->query("select id_category from category_children where (id_children='".$this->id."' and children_type='shop')");
                    ...
                    case "products":
          						$list = array();
          						$q=$db->query("select id from product where (id_page='".$this->id."' and page_type='shop')");
          						while($r=$q->fetch_row()) $list[] = new product($r[0]);
          						return $list;
          					break;
          					case "services":
          						$list = array();
          						$q=$db->query("select id from service where (id_page='".$this->id."' and page_type='shop')");
                    ...
                    case "is_contracted":
                        $q=$db->query("select count(id) from contract where (id_page='".$this->id."' and page_type='shop' and TIMESTAMPDIFF(DAY,NOW(),DATE_ADD(creation_time, INTERVAL duration MONTH)) >= 0)");
                        $r=$q->fetch_row();
                        return $r[0]>0;
                    break;
                    case "current_contract":
                        $q=$db->query("select id from contract where (id_page='".$this->id."' and page_type='shop' and TIMESTAMPDIFF(DAY,NOW(),DATE_ADD(creation_time, INTERVAL duration MONTH)) >= 0) ORDER BY DATE_ADD(creation_time, INTERVAL duration MONTH) DESC LIMIT 1");
