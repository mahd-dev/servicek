		...
		public function __get($name){
			...
			if ($this->id != NULL) {
				switch($name){
					...
					case "is_contracted":
						$q=$db->query("select count(id) from contract where (id_page='".$this->id."' and page_type='company' and TIMESTAMPDIFF(DAY,NOW(),DATE_ADD(creation_time, INTERVAL duration MONTH)) >= 0)");
						$r=$q->fetch_row();
						return $r[0]>0;
					break;					
					case "current_contract":
						$q=$db->query("select id from contract where (id_page='".$this->id."' and page_type='company' and TIMESTAMPDIFF(DAY,NOW(),DATE_ADD(creation_time, INTERVAL duration MONTH)) >= 0) ORDER BY DATE_ADD(creation_time, INTERVAL duration MONTH) DESC LIMIT 1");
